...
- **Home page categories block** also reads `menuRoots` from `/api/menu` and renders an **image-free typographic editorial list** (numbered rows: number · title · sub-category names · arrow), since the new Trendyol-imported leaf categories don't have curated category images.
