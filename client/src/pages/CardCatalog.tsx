...
export default function CardCatalog() {
  ...
  const setFilter = useCallback((updates: Record<string, string | null>, resetPage = true) => {
    ...
    navigate('/kartlar' + (p.toString() ? '?' + p.toString() : ''), { replace: true });
  }, [searchStr, navigate]);
          ...
          <main className="flex-1 min-w-0">
            ...
            <div className="mb-6">
              <h1 className="text-xl font-bold text-white/90 tracking-wide">
                {selectedGame
                  ? (games.find(g => g.slug === selectedGame)?.name ?? selectedGame) + ' Kartları'
                  : 'Tüm Kartlar'}
              </h1>
              {selectedSet && sets.length > 0 && (
                <p className="text-sm text-white/40 mt-0.5">
                  {sets.find(s => s.slug === selectedSet)?.name ?? selectedSet}
                </p>
              )}
            </div>
              ...
              <div className="flex items-center gap-3 mb-5">
                <p className="text-sm text-white/35">
                  {isLoading ? 'Yükleniyor...' : (
                    total > 0
                      ? <>{total.toLocaleString('tr-TR')} kart bulundu</>
                      : 'Sonuç bulunamadı'
                  )}
                </p>
              </div>
