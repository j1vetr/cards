...
function FaqAccordion({ items }: { items: FaqItem[] }) {
  ...
  return (
    <div className="mt-10 border-t border-white/10 pt-8" data-testid="faq-section">
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="border border-white/10 rounded-xl overflow-hidden" data-testid={`faq-item-${i}`}>
            <button
              type="button"
              onClick={() => setOpenIdx(openIdx === i ? null : i)}
              className="w-full flex items-center justify-between px-4 py-3.5 text-left text-[14px] font-semibold text-white hover:bg-[#0e1424]/5 transition-colors"
              aria-expanded={openIdx === i}
              data-testid={`button-faq-toggle-${i}`}
            >
              <span>{item.question}</span>
              <ChevronDown className={`w-4 h-4 text-white/45 shrink-0 ml-3 transition-transform ${openIdx === i ? 'rotate-180' : ''}`} />
            </button>
            {openIdx === i && (
              <div className="px-4 pb-4 text-[14px] text-white/65 leading-relaxed border-t border-white/5 pt-3" data-testid={`text-faq-answer-${i}`}>
                {item.answer}
              </div>
            )}
          </div>
...
function RelatedPostCard({ post }: { post: BlogPost }) {
  return (
    <Link href={`/blog/${post.slug}`}>
      <div
        className="group flex gap-3 p-3 rounded-xl border border-white/10 hover:border-indigo-200 hover:shadow-sm transition-all bg-[#0e1424]"
        data-testid={`card-related-${post.id}`}
      >
        <div className="w-16 h-16 rounded-lg overflow-hidden shrink-0 bg-[#0e1424]/5">
          {post.coverImageUrl ? (
        ...
        <div className="min-w-0">
          ...
          <p className="text-[12px] font-bold text-white leading-snug group-hover:text-indigo-700 transition-colors line-clamp-2 mt-0.5">
            {post.title}
          </p>
          {post.publishedAt && (
            <p className="text-[10px] text-white/45 mt-1">{formatDate(post.publishedAt)}</p>
          )}
        </div>
      </div>
    </Link>
      ...
      <main className="min-h-screen bg-[#080e1c]">
        {isLoading && (
          <div className="max-w-4xl mx-auto px-4 py-16 animate-pulse space-y-4">
            <div className="h-4 bg-[#0e1424]/10 rounded w-1/3" />
            <div className="h-8 bg-[#0e1424]/10 rounded w-3/4" />
            <div className="h-[360px] bg-[#0e1424]/10 rounded-2xl" />
            <div className="space-y-2 pt-4">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-4 bg-[#0e1424]/10 rounded" style={{ width: `${75 + Math.random() * 25}%` }} />
              ))}
            </div>
          </div>
        )}
        ...
        {isError && (
          <div className="max-w-3xl mx-auto px-4 py-20 text-center">
            ...
            <p className="text-white/65 font-medium mb-1">Yazı bulunamadı</p>
            <p className="text-white/45 text-sm mb-6">Bu yazı mevcut değil veya kaldırılmış olabilir.</p>
        ...
        {post && (
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
            ...
            {/* Breadcrumb */}
            <nav className="flex items-center gap-1.5 text-[12px] text-white/45 mb-6 flex-wrap">
              <Link href="/" className="hover:text-white/75 transition-colors">Ana Sayfa</Link>
              <ChevronRight className="w-3 h-3" />
              <Link href="/blog" className="hover:text-white/75 transition-colors">Blog &amp; Rehber</Link>
              <ChevronRight className="w-3 h-3" />
              <span className="text-white/65 truncate max-w-[240px]">{post.title}</span>
            </nav>
            ...
            <div className={`flex flex-col ${hasSidebar ? 'lg:flex-row' : ''} gap-10`}>
              ...
              <article className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2.5 mb-4 flex-wrap">
                  <span className={`text-[11px] font-bold uppercase tracking-widest px-3 py-1 rounded-full border ${CATEGORY_COLORS[post.category] ?? 'bg-[#0e1424]/5 text-white/65 border-white/10'}`}>
                    {CATEGORY_LABELS[post.category] ?? post.category}
                  </span>
                  {post.publishedAt && (
                ...
                {/* Title */}
                <h1 className="text-2xl sm:text-[32px] font-bold text-white leading-tight mb-5">
                  {post.title}
                </h1>
                ...
                <div className="flex items-center gap-3 mb-6 pb-6 border-b border-white/10">
                  ...
                  <div>
                    <p className="text-[13px] font-semibold text-white/90">GoCards Editörü</p>
                    <p className="text-[11px] text-white/45">GoCards</p>
                  </div>
                </div>
                ...
                {/* Summary */}
                {post.summary && (
                  <p className="text-[16px] text-white/65 leading-relaxed mb-8 border-l-4 border-indigo-300 pl-4 italic">
                    {post.summary}
                  </p>
                )}
                ...
                <div
                  className="prose prose-invert prose-base max-w-none
                    ...
                    prose-p:text-white/75 prose-p:leading-relaxed prose-p:text-[15px]
                    prose-li:text-white/75 prose-li:text-[15px]
                    prose-strong:text-white
                    prose-blockquote:border-indigo-300 prose-blockquote:text-white/65 prose-blockquote:not-italic
                ...
                <aside className="lg:w-72 shrink-0">
                  <div className="sticky top-20">
                    <div className="flex items-center gap-2 mb-3">
                      <Tag className="w-3.5 h-3.5 text-white/45" />
                      <h3 className="text-[12px] font-bold text-white/55 uppercase tracking-widest">
